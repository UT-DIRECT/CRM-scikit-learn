...
def plot_parameter_convergence_fitting():
    ...
    for i in range(len(producers)):
        ...
        producer = i + 1
        producer_rows_df = q_fitting_sensitivity_analysis_df.loc[q_fitting_sensitivity_analysis_df['Producer'] == producer]
...
def plot_parameter_convergence_prediction():
    ...
    for i in range(len(producers)):
        plt.figure(figsize=[7, 4.8])
        producer = i + 1
        producer_rows_df = q_predictions_sensitivity_analysis_df.loc[q_predictions_sensitivity_analysis_df['Producer'] == producer]
        x_i = producer_rows_df['f1_initial']
        x_f = producer_rows_df['f1_final']
        y_i = producer_rows_df['tau_initial']
        y_f = producer_rows_df['tau_final']
